...
internal static class RectilinearPolygonSolver {
    private static Color[] colors = null!;
    private static bool[,] covered = null!;
    private static Size2D size;
    private static List<Rect8>? current;
    private static List<Rect8>? best;
    ...
    private static bool NeedFilling(int x, int y) => y < size.Length && !covered[x, y] && colors[y * size.Width + x].A > 0;

    private static int Rightest(int x, int y) {
        while (x < size.Width && NeedFilling(x, y)) {
            ++x;
        }
    ...
    private static void Dfs(int x1, int y1) {
        if (best is not null && current?.Count >= best.Count) {
            return;
        }

        while (y1 < size.Length) {
            while (x1 < size.Width) {
                if (NeedFilling(x1, y1)) {
                    goto search;
                }
    ...
    search:
        ...
        for (var y2 = y1 + 1; rightest > x1 && y2 <= size.Length; ++y2) {
            ...
            if (x2 >= rightest) {
                Cover(x1, rightest, y2);
            } else {
                current?.Add(new(x1, y1, rightest - x1, y2 - y1));
                Dfs(x1 + 1, y1);
